    ...
    <div class="container-fluid">
        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
                ...
                @endif
                <form class="row form" method="post" action="{{ route('services.update',$service->id) }}"
                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="service_id" id="service_id">
                                ...
                                @foreach(Service::getParentService() as $ser)
                                @if($ser->id!=$service->id)
                                <option value="{{ $ser->id }}" {{ ($service->service_id==$ser->id)?'selected':'' }}>
                                    {{ $ser->title }}</option>
                                @endif
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="position" name="position"
                                placeholder="Enter position" value="{{ $service->position }}">
                        </div>
                        <div class="form-group">
                            ...
                            <div class="d-flex mt-2">
                                <img src="@if($service->image) {{ asset('storage/'.$service->image) }} @endif"
                        ...
                        <div class="card card-success">
                            ...
                            <div class="card-body">
                                ...
                                <input type="text" class="form-control" id="met_title" name="meta_title"
                                    placeholder="Enter Meta Title" value="{{ $service->seo['meta_title']??'' }}">
                            </div>
                            <!-- /.card-body -->
                            <div class="card-body">
                                <label>Meta Description</label>
                                <textarea class="form-control" name="meta_description" rows="5"
                                    placeholder="Enter Meta Description">{{ $service->seo['meta_description']??'' }}</textarea>
                            </div>
                        </div>
                        <div class="card card-info">
                            ...
                            <div class="card-body">
                                <input type="checkbox" name="status" value="1" data-bootstrap-switch
                                    data-off-color="danger" data-on-color="success" {{ ($service->status)?'checked':'' }}>
                            </div>
                        </div>
...
<script>
        $('.service-menu').removeClass('menu-is-opening menu-open');
            $('.service-menu').addClass('menu-is-opening menu-open');
            $('.service-menu').find('>:first-child').removeClass('active');
            $('.service-menu').find('>:first-child').addClass('active');
    $(function(){
        $('.select2').select2({
        theme: 'bootstrap4'
        })
