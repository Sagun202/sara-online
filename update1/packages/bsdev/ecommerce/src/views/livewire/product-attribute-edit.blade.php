<div>
    ...
    <input type="hidden" wire:model="attribute_values.{{ $i }}.id" value="{{ $val['id']??$key }}">
    <div class="row" style="border: 2px solid red; padding:20px;">
        ...
        <div class="col-6">
            ...
            <div style="display: flex; flex-wrap:wrap;">
                @foreach ($attribute_values[$i]['images']??[] as $keyImage=>$image)
                <div style="display: flex; flex-direction:column; padding-right:5px;">
                    <img src="{{ asset('storage/'.$image) }}" style="height: 100px" width="100px">
                    <a href="javascript:void(0);" wire:click="removeImage({{ $i }},{{ $keyImage }})"
                        class="btn btn-danger" style="font-size: smaller !important;">remove</a>
                </div>
                @endforeach
            </div>
