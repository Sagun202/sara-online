...
class ProductAttributeCreate extends Component
{
    use WithFileUploads;
    ...
    {
        return [
            'attributes_values.*.title' => 'required|string|max:255',
            'attributes_values.*.quantity' => 'required|number',
            'attributes_values.*.media' => 'nullable|mimes:jpg,png,jpeg,gif',
            'attributes_values.*.conf.*' => 'required',
        ];
    }

    public function render()
    {
        return view('ecommerce::livewire.product-attribute-create');
    }
    ...
    public function updated($propertyName)
    {
        ...
        // dd(json_encode($this->attribute_values));
    ...
    public function save()
    {
        ...
        foreach ($this->attribute_values as $key => $val) {
            ...
            if (count($val['conf']) != count($this->selectedAttributes)) {
