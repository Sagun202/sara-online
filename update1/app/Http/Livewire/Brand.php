...
class Brand extends Component
{
    ...
    {
        $this->products = Product::where('brand_id', $this->brand->id)->where('status', 1)->with('categories', function ($query) {
            $query->where('status', 1);
        })->get();
        $categories = $this->products->pluck('categories')->flatten()->pluck('id')->toArray();
        $brand = $this->brand->id;
        $this->categories = Category::whereIn('id', $categories)->where('status', 1)->with(['products' => function ($query) use ($brand) {
            $query->where('status', 1)->where('brand_id', $brand);
        }])->get();
